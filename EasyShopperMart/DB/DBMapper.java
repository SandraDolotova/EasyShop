...
public class DBMapper {
    ...
    public static String checkSalesRepresentativeName() throws SQLException, ClassNotFoundException {

        Connection connection = DBConnection.DBConnection();
        String checkSalesRepresentativeName = "";
        if (connection != null) {
            try {
                ...
                checkSalesRepresentativeName = stringBuilder.toString();
                connection.close();
            } catch (Exception exc) {
                exc.printStackTrace();
            }
        }
        return checkSalesRepresentativeName;

    }
    public static String checkSalesRepresentativePass() throws SQLException, ClassNotFoundException {

        Connection connection = DBConnection.DBConnection();
        String checkSalesRepresentativePass = "";
        if (connection != null) {
            try {
                PreparedStatement preparedStatement = DBConnection.DBConnection().prepareStatement(sql6);
                ResultSet resultSet = preparedStatement.executeQuery(sql6);
                StringBuilder stringBuilder = new StringBuilder();
                while (resultSet.next()) {
                    stringBuilder.append(resultSet.getString("user_password"));
                }
                checkSalesRepresentativePass = stringBuilder.toString();
                connection.close();
            } catch (Exception exc) {
                exc.printStackTrace();
            }
        }
        return checkSalesRepresentativePass;

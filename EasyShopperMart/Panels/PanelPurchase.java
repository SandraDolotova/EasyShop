...
public class PanelPurchase extends PanelStore {
    ...
    public static JPanel createPurchasePanel() {
    ...
    public static void changeProductQuantity() throws SQLException, ClassNotFoundException {
        ...
        if (connection != null) {
            try {
                // String sql = "UPDATE sales_products SET product_quantity = ((SELECT (product_quantity) FROM sales_products WHERE product_id = ' " + productID + " ' ) - (SELECT (product_quantity) FROM shopping_cart WHERE product_id = ' " + productID + " ' )) ";
                String sql = " UPDATE sales_products SET product_quantity_stock = product_quantity_stock - (SELECT (product_quantity) FROM shopping_cart WHERE product_id = ' " + id + " ' ) WHERE product_id = ' " + id + " '";
